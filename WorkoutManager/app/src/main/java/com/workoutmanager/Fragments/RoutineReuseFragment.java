...
public class RoutineReuseFragment extends android.support.v4.app.Fragment implements DataHandler {
    ...
    public void getData() {
        mainViewModel.getRoutineId().observe(getActivity(), new Observer<Routine>() {
            ...
            public void onChanged(@Nullable Routine routineId) {
                Call<List<User>> starsList = retrofit.createClient().reuseRoutineUserList(routineId.getId());
